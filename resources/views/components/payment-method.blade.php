...
@php
    ...
    $title = $title ?? __('qredit-sdk::checkout.title');
    $desc  = $description ?? __('qredit-sdk::checkout.description');
...
<label class="qredit-card {{ $isSelected ? 'qredit-selected' : '' }}" dir="{{ $dir }}" id="qredit-payment-card" onclick="document.getElementById('qredit-radio-input').checked=true;this.classList.add('qredit-selected')">
    ...
    <div class="qredit-info">
        <div class="qredit-info-row">
            ...
            <span>{{ __('qredit-sdk::checkout.secure-encrypted') }}</span>
        </div>
        <div class="qredit-info-row">
            ...
            <span>{{ __('qredit-sdk::checkout.card-qr') }}</span>
        </div>
        <div class="qredit-info-row">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
            <span>{{ __('qredit-sdk::checkout.no-hidden-fees') }}</span>
        </div>

        @if ($isSandbox)
            <div class="qredit-sandbox-badge">{{ __('qredit-sdk::checkout.test-mode') }}</div>
        @endif
