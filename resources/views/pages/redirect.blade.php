...
<head>
    ...
    <title>{{ __('qredit-sdk::checkout.redirecting') }}</title>
...
<body>
    <div class="qredit-redirect">
        ...
        <div class="qredit-redirect__card">
            ...
            <h1 class="qredit-redirect__title">{{ __('qredit-sdk::checkout.redirecting') }}</h1>
            <p class="qredit-redirect__subtitle">{{ __('qredit-sdk::checkout.please-wait') }}</p>

            @if ($amount || $ref)
                <div class="qredit-redirect__chips">
                    @if ($amount)
                        <div class="qredit-redirect__chip">
                            <span class="qredit-redirect__chip-label">{{ __('qredit-sdk::checkout.amount') }}</span>
                            {{ $amount }}
                        </div>
                    @endif
                    @if ($ref)
                        <div class="qredit-redirect__chip">
                            <span class="qredit-redirect__chip-label">{{ __('qredit-sdk::checkout.reference') }}</span>
                            {{ $ref }}
                        </div>
                    @endif
                </div>
            ...
            <p class="qredit-redirect__warn">{{ __('qredit-sdk::checkout.do-not-refresh') }}</p>

            <a href="{{ $redirectUrl }}" class="qredit-redirect__link">
                {{ __('qredit-sdk::checkout.click-here') }}
            </a>
        </div>

        <div class="qredit-redirect__footer">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
            {{ __('qredit-sdk::checkout.secure-footer') }}
        </div>
    </div>
