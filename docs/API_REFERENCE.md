...
### `Qredit::redirectUrls(): RedirectUrlStore`

Access the bound redirect-URL store. Mostly useful for tests and runtime swaps.

### `Qredit::rememberRedirectUrls(string $paymentReference, QreditRedirectUrls $urls, ?string $tenantId = null): void`

Persist the four widget callback URLs (success/cancel/failure/pending) keyed by the gateway's payment reference. Pass an explicit `$tenantId` in queue jobs.

```php
use Qredit\LaravelQredit\Tenancy\QreditRedirectUrls;

Qredit::rememberRedirectUrls('66573792', QreditRedirectUrls::fromArray([
...
### `Qredit::resolveRedirectUrls(string $paymentReference, ?string $tenantId = null): ?QreditRedirectUrls`

Look up previously-stored URLs. Returns `null` when nothing is stored or the entry has expired. See [REDIRECT_URLS.md](REDIRECT_URLS.md) for the full design.

### `Qredit::forgetRedirectUrls(string $paymentReference, ?string $tenantId = null): void`
